...
const handleCategoryChange = async (file, newCategoryId) => {
  // پیدا کردن کل آبجکت دسته‌بندی بر اساس id
  const newCategoryObj = categories.find(cat => cat.id === newCategoryId) || null;

  // آپدیت دسته‌بندی در دیتابیس
  await updateFileCategory(file.id, newCategoryObj);

  // آپدیت لیست آهنگ‌ها در state به صورت محلی (بدون دوباره گرفتن از DB)
  setAudioFiles(prevAudioFiles => 
    prevAudioFiles.map(f => 
      f.id === file.id ? { ...f, category: newCategoryObj } : f
    )
  );

  // اگر موزیک در حال پخش بود و دسته‌بندی‌اش تغییر کرد، اونم بروز کن
