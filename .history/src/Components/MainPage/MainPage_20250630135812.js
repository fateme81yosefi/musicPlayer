...
const MainPage = () => {
    ...
    const addPlaylist = async (playlist) => {
        ...
        const playlistWithId = {
            ...playlist,
            id: playlist.id || generateRandomId(), // فقط اگر id نداشت، بساز
        };

        await db.put('playlists', playlistWithId);
    ...
    const handleAddCategory = async () => {
        if (category) {
            ...
            const saveCategory = async (cover) => {
                ...
                setCategories((prev) => [...prev, newPlaylist]);
                setCategory('');
                setCoverImage(null);
                setPreviewUrl('');
                setShowModal(false);
        ...
        const db = await openDB(DB_NAME, DB_VERSION, {
            upgrade(db) {
                if (!db.objectStoreNames.contains(STORE_NAME)) {
                    ...
                    // اندیس‌ها اضافه کن فقط اگه وجود ندارن
                    if (!store.indexNames.contains('category')) {
                        store.createIndex('category', 'category');
                    }
                    if (!store.indexNames.contains('isFav')) {
