...
const MainPage = () => {
    ...
    const handleDeleteAll = async () => {
        const updatedList = await deleteAllAudioFiles();
        setAudioFiles(updatedList); // یا هر استیتی که داری
    };
    ...
    const handleCategoryChange = async (file, newCategoryId) => {
        const newCategoryObj = categories.find(cat => cat.id === newCategoryId) || null;

        await updateFileCategory(file.id, newCategoryObj);
        await fetchAndSetAudioFiles();


        const updatedAudioFilesRaw = await getStoredFiles();
        const filesWithUrl = updatedAudioFilesRaw.map((file) => ({
            ...file,
            data: URL.createObjectURL(new Blob([file.data], { type: file.type })),
        }));
        ...
        <div className="container">
            ...
            <div className="rightPage">
                ...
                <div className='fullWidth headerBtnContainer'>
                    <div className=''>
                        <button className='uploadBtnHeader marginer' onClick={handleDeleteAll}>Delete All Musics</button>
