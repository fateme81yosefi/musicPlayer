...
import { deleteAllAudioFiles } from '../../db/audioDB';
...
const MainPage = () => {
    ...
    const handleDelete = async (fileId) => {
        ...
        // اگر موزیک در حال پخش حذف شد، currentMusic رو پاک کن
    ...
    const handleCategoryChange = async (file, newCategory) => {
        // آپدیت دسته‌بندی در DB (باید تابع updateFileCategory رو import کرده باشی)
        await updateFileCategory(file.id, newCategory);

        // بازخوانی لیست موزیک‌ها (باید یک تابع fetchAudioFiles داشته باشی یا مشابه)
        // این تابع رو می‌تونی از MusicLibrary یا AudioList هم بیاری یا داخل MainPage تعریف کنی
        const updatedAudioFiles = await getStoredFiles(); // فرض بر این که getStoredFiles داده‌ها رو کامل برمی‌گردونه
        const filesWithUrl = updatedAudioFiles.map((file) => ({
            ...file,
            data: URL.createObjectURL(new Blob([file.data], { type: file.type })),
        }));
        ...
        // اگر موزیک در حال پخش، همون موزیک بود، currentMusic رو آپدیت کن که تغییر کتگوری اعمال بشه و پخش قطع نشه
        if (currentMusic?.id === file.id) {
            // پیدا کردن موزیک آپدیت شده در لیست جدید
            ...
            <div className="rightPage">
                ...
                <div className='row'>
                    ...
                    <MusicLibrary
                        ...
                        onCategoryChange={handleCategoryChange}  // اضافه کردن این prop
                    />
                </div>
            </div>
        </div>
