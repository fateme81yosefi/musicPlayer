...
const handleCategoryChange = async (file, newCategoryId) => {
  // 1. دسته جدید رو از لیست categories پیدا کن
  const newCategory = categories.find(cat => cat.id === newCategoryId);

  // 2. اگر دسته پیدا شد، آپدیت کن
  if (!newCategoryId || !newCategory) {
    // اگر مقدار خالی بود یا دسته پیدا نشد، ممکنه بخوای دسته رو پاک کنی یا کاری انجام بدی
    await updateFileCategory(file.id, null); // یا هر کاری لازمه
  } else {
    await updateFileCategory(file.id, newCategory);
  }

  // 3. بروز رسانی لیست موزیک‌ها
  const updatedAudioFilesRaw = await getStoredFiles();
  const filesWithUrl = URL.createObjectURL(updatedAudioFilesRaw);
  setAudioFiles(filesWithUrl);

  // 4. آپدیت موزیک در حال پخش در صورت نیاز
