...
const deleteMusicById = async (id) => {
  try {
    ...
    await store.delete(id); // ❗ حذف آیتم
    await tx.done;

    console.log(`✅ آیتم با id ${id} از دیتابیس حذف شد`);
  } catch (error) {
    console.error("❌ خطا در حذف از IndexedDB:", error);
  }
};
              ...
              <div className="row aligner">
                <button
                  onClick={() => {
                    ...
                    setAudioFiles((prev) => prev.filter((m) => m.id !== file.id));
                  }}
