...
const AudioList = ({
  ...
  const handleDelete = async (fileId) => {
    ...
    // اگر موزیک در حال پخش حذف شده بود، currentMusic را پاک کن
  ...
  const handleCategoryChange = async (file, categoryName) => {
    ...
    if (file.id) {
      ...
      // اگر فایل تغییر یافته همان فایلی است که در حال پخش است، currentMusic را به‌روزرسانی کن
      if (currentMusic?.id === file.id) {
        // آبجکت جدید فایل را از دیتابیس بگیر و به currentMusic بفرست
        const updatedFiles = await getStoredFiles();
        const updatedFile = updatedFiles.find((f) => f.id === file.id);
        if (updatedFile) {
          const updatedFileWithUrl = {
            ...updatedFile,
            data: URL.createObjectURL(new Blob([updatedFile.data], { type: updatedFile.type })),
          };
          setCurrentMusic(updatedFileWithUrl);
        }
      }
    } else {
