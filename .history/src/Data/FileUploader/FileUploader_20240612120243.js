import React, { useStateوuseEffect } from 'react';
import { useDropzone } from 'react-dropzone';
import "./FileUploader.css"

const FileUploader = () => {
    const [audioFiles, setAudioFiles] = useState([]);

    const onDrop = (acceptedFiles) => {
        const mp3Files = acceptedFiles.filter((file) =>
            file.name.endsWith('.mp3')
        );

        setAudioFiles(mp3Files);
    };

    const { getRootProps, getInputProps } = useDropzone({ onDrop });

    useEffect(() => {
        // در زمان بارگیری کامپوننت، فایل‌های صوتی را از localStorage بخوانید
        const storedAudioFiles = JSON.parse(localStorage.getItem('audioFiles'));
        if (storedAudioFiles) {
          setAudioFiles(storedAudioFiles);
        }
      }, []);
    
      // تابعی برای اضافه کردن فایل صوتی به لیست
      const handleAddAudioFile = (file) => {
        const updatedAudioFiles = [...audioFiles, file];
        setAudioFiles(updatedAudioFiles);
    
        // فایل‌های صوتی را در localStorage ذخیره کنید
        localStorage.setItem('audioFiles', JSON.stringify(updatedAudioFiles));
      };

    return (

        
            audioFiles.length !== 0 ?


                (audioFiles.map((file, index) => (
                    <div className='singleSelectedAudio'>
                        <div className='row'>
                            <span>{file.name}</span>
                            <button>play</button>
                        </div>
                        <audio className='selectedAudio' src={URL.createObjectURL(file)} key={index} controls />

                    </div>
                ))
                )
                :
                (<div {...getRootProps()}>
                    <input {...getInputProps()} />

                    <div className='chooseFile'>
                        <img alt='drag file' src='/images.png' />
                        <p>choose file</p>
                    </div>


                </div >)

        

    );
};

export default FileUploader;
